...
async function verifyData() {
    try {
        ...
        console.log(`\nDocuments found with book=4 and unit="3-4": ${querySnapshot.size}`);
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (data.book === 4 && data.unit === '3-4' && data.errorCount === 0) {
                correctCount++;
            }
        });

        console.log(`Documents with correct tags (book: 4, unit: "3-4", errorCount: 0): ${correctCount}`);

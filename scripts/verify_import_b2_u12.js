...
async function verifyData() {
    try {
        ...
        console.log(`\nDocuments found with book=2 and unit="1-2": ${querySnapshot.size}`);
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (data.book === 2 && data.unit === '1-2' && data.errorCount === 0) {
                correctCount++;
            }
        });

        console.log(`Documents with correct tags (book: 2, unit: "1-2", errorCount: 0): ${correctCount}`);

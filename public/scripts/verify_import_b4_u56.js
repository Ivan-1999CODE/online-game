...
async function verifyData() {
    try {
        ...
        console.log(`\nDocuments found with book=4 and unit="5-6": ${querySnapshot.size}`);
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (data.book === 4 && data.unit === '5-6' && data.errorCount === 0) {
                correctCount++;
            }
        });

        console.log(`Documents with correct tags (book: 4, unit: "5-6", errorCount: 0): ${correctCount}`);

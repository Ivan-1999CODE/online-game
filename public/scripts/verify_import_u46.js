...
async function verifyData() {
    try {
        ...
        console.log(`\nDocuments found with unit="4-6": ${querySnapshot.size}`);

        if (querySnapshot.empty) {
            console.log('No documents found for unit 4-6!');
            process.exit(0);
        }
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            if (data.book === 1 && data.errorCount === 0) {
                correctCount++;
            }
        });

        console.log(`Documents with correct tags (book: 1, errorCount: 0): ${correctCount}`);
